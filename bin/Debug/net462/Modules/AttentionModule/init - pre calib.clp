...
(defglobal ?*KinectFOV_hor* = 42.05) 
(defglobal ?*KinectFOV_ver* = 26.9) 
...
(?x_coord ?z_coord)
   (bind ?X_max (* ?z_coord (tan (/ ?*KinectFOV_hor* (* 180 ?*pi*)))))
   (bind ?x (+ 0.5 (/ ?x_coord (/ ?X_max 2))))
   (return ?x)
)
...
(?y_coord ?z_coord)
   (bind ?Y_max (* ?z_coord (tan (/ ?*KinectFOV_ver* (* 180 ?*pi*)))))
   (bind ?y (+ 0.5 (/ ?y_coord (/ ?Y_max 2))))
   (return ?y)
)

;Xmax = subject.head.Z * (float)Math.Tan(42.05 / 180.00 * Math.PI); //alzando l'angolo della tangente aumento l'eccentricità 
;                        X = subject.head.X / Xmax / 2 + (float)0.5;
...
(defrule MAIN::look_at_winner "Finally, this rule makes the robot look at the winner making the expression that is the most suitable to the current social context"
   ...
   (printout t "VINCITORE [ "?id" | "?x_appr" | "?y_appr" ] ECS ( "?new_ev" | "?new_ea" )  [ "?rulefired" ]"  crlf)
   (fun_lookat ?id ?x_appr ?y_appr)
   (fun_makeexp 0.45 0.8)
