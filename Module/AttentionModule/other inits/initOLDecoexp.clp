...
(defrule MAIN::moodrule_empathy "This rule makes the robot happy if the subject is smiling"
   ...
   (test (> ?happy 0.7))
   =>
   (bind ?newv (moodchange ?v (/ ?happy 1900)))
   (bind ?newa (moodchange ?a (/ ?happy 1800)))
...
(defrule MAIN::moodrule_not_engaged "This rule makes the robot sad if the subject is not Engaged"
   ...
   =>
   (bind ?newv (moodchange ?v -0.009))
   (bind ?newa (moodchange ?a -0.004))
   (modify ?face (mood ?newv ?newa))
...
(defrule say_the_empty_sentence
   ...
   (bind ?n (random 1 4))
   (fun_speech ?n)
   (if (or (eq ?n 1) (eq ?n 2)) then (bind ?timer 57))
   (if (or (eq ?n 3) (eq ?n 4)) then (bind ?timer 18))
   (assert (speak_timer ?timer))
   (assert (wait_for sentence_2))
   (modify ?ecoexp (event EMPTY_sentence))
)
...
(defrule say_the_promising_sentence
   ...
   (bind ?n (random 5 8))
   (fun_speech ?n)
   (if (or (eq ?n 5) (eq ?n 6)) then (bind ?timer 57))
   (if (eq ?n 7) then (bind ?timer 18))
   (if (eq ?n 8) then (bind ?timer 24))
   (assert (speak_timer ?timer))
   (assert (wait_for sentence_2))
   (modify ?ecoexp (event PROMISING_sentence))
)
